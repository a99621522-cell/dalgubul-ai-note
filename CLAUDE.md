...
- `config/pledge_areas.yml` + `scripts/report_context.py` — 매주 산업별 정책제안 리포트 루틴(docs/ROUTINE_PROMPT.md, Claude 루틴)의 분야 10개(6대 산업 + 기계·자동차·섬유·뿌리산업, 분야마다 주 1편 — `weekday_plan` 으로 평일 2개씩, 분야별 `ksic`·`focus`)와 근거 계산(기업 사전·사업 DB·대구시 매칭·최근 글). 리포트에 예산액·사업코드·예산서를 인용하지 않는다(정책제안서, 운영자 지시 2026-09-25 — 정부·시 사업은 사업명·신규 여부만). 공약 이행 평가·점수화 금지
