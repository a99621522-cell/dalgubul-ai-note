...
- `config/pledge_areas.yml` + `scripts/report_context.py` — 매주 산업별 정책제안 리포트 루틴(docs/ROUTINE_PROMPT.md, Claude 루틴. 루틴이 운영 세션을 깨우면 운영 세션이 지정 브랜치 `claude/policy-reports` 를 가진 작성 세션을 만든다 — 루틴이 직접 만든 세션은 푸시가 승인 대기로 막힌다)의 분야 10개(6대 산업 + 기계·자동차·섬유·뿌리산업, 분야마다 주 1편 — `weekday_plan` 으로 평일 2개씩, 분야별 `ksic`·`focus`)와 근거 계산(기업 사전·사업 DB·대구시 매칭·최근 글). 리포트에 예산액·사업코드·예산서를 인용하지 않는다(정책제안서, 운영자 지시 2026-09-25 — 정부·시 사업은 사업명·신규 여부만). 공약 이행 평가·점수화 금지
- `config/research_sources.yml` + `scripts/fetch_research.py` + `.github/workflows/research.yml` — 정책제안 리포트 근거 창고. 국내 연구기관(KIET·KDI·KISTEP·STEPI·NABO·한국은행·대구정책연구원·KDB·KITA·KOTRA·KOSI)·기업지원기관(KEIT·KIAT·IITP·NIPA·NIA·KISTI·KIRIA·중진공)·대구 기관·정부 보도자료·해외 협회(IFR·SEMI·SIA·IEA·OECD·ACEA)·해외 컨설팅(McKinsey·BCG·Deloitte·Gartner·IDC)·해외 정부의 최근 70일 발간물 제목·링크·날짜·요약 300자를 RSS(자동 발견) 또는 목록 페이지에서 받아 `data/research/<key>.json` + `summary.md`(접속 결과표). 매주 월 03:00 KST. robots.txt 존중, 본문 저장 안 함. `report_context.py` 가 분야 키워드 일치 항목을 출력하고 `fetch_research.py --query` 로 검색. 이 세션 환경은 외부 사이트가 모두 막혀 있어 워크플로만 받을 수 있다. 국내 발간 목록은 공공데이터포털 파일데이터(`datago:` — KEIT PD이슈리포트 15047479·이슈리뷰 15121599, KIAT 정책간행물 15104286, KISDI 3071323, 북구 보도자료 15149016)로도 받는다. 제목이 링크가 아닌 행 클릭형 게시판은 날짜 든 칸의 글자에서 제목만 뽑고 링크는 목록 페이지로 건다(`from_rows`). 대구시 뉴스룸(info.daegu.go.kr)은 robots.txt 차단이라 시청 첫 화면 주요뉴스만. 미국 러너에서 접속이 안 되는 대구 기관·KISTI·KIRIA·한국은행은 국내 self-hosted 러너(라벨 korea, 워크플로 `research_kr.yml`, 수동 실행)나 국내 PC 에서 `--only` 로 받아 커밋한다
- `scripts/review_report.py` + `.github/workflows/review_reports.yml` — 정책제안 리포트 품질 검사(구조 1~9절·제안 3개×여섯 항목·문단마다 숫자·금지 형용사·평가 표현·기업 지목·출처 수·길이·출처 최신성 — 날짜 미확인 출처와 1년 넘은 출처는 오류, 최근 120일 안 출처 60% 미만은 경고. 옛 자료로 정책 제안을 쓰지 않는다, 운영자 지시 2026-09-26). 작성 루틴이 통과시킨 뒤 자동 발행하고, main 푸시 때 워크플로가 발행본을 다시 검사해 미달이면 `draft: true` 로 되돌리고 재배포. 기준을 바꿀 때는 발행된 리포트 전체(`--published`)가 통과하는지 확인
