...
- `config/pledge_areas.yml` + `scripts/report_context.py` — 매주 산업별 정책제안 리포트 루틴(docs/ROUTINE_PROMPT.md, Claude 루틴)의 분야 순환표 10개(6대 산업 + 기계·자동차·섬유·뿌리산업, `week_offset` 로 시작 주 조정, 분야별 `ksic`·`focus`)와 근거 계산(기업 사전·사업 DB·대구시 매칭·최근 글). 리포트는 `draft: true`, 공약 이행 평가·점수화 금지
