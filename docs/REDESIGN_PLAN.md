...
- 메뉴: 현황 / 산업별 / 기업 사전 / 사업·예산(/programs/) / 지원 기업(/support/, 2026-09-25 추가: 공무원용 전환으로 지원받은 기업 목록이 필요해짐) / 글. 첫 화면 4번째 타일 = 지원사업 수혜 기업(최근 3년)
...
- 만든 페이지: `/industry/`(표·막대·그룹 카드), `/industry/[key]/`(타일 4·24개월 추이·단지/입지 분포·지원 이력), `/industry/compare/`, `/dashboard/`(타일 4·입지 유형·구군 9·단지 21+개별입지/산단 외·태그), `/stats/[month]/`, `/support/`, `/posts/`
- 기업 사전 목록: 입지 유형·태그·지원 이력 필터, 정렬(가나다/규모/구군), 100곳 단위 더 보기. 색인 `companies-index.json` 에 `p`(지원 이력) 추가. `IndustrialCard` 는 목록·분류 페이지에서 뺐다(현황판이 대신)
- 기업 카드: 타일 3개(고용 인원·지원 이력 3년·12개월 스파크라인) + 입지·태그 배지 줄
- 화면 확인: 1280·390 스크린샷(홈·현황판·산업별·그룹·비교·기업 사전·카드·지원 기업·통계표·글). Lighthouse 모바일: 접근성 100(6페이지 전부), SEO 91~100, 성능 78~96
- 남은 것: `write_monthly_report.py`(월보), 성능(LCP 3초대: 폰트 프리로드·표 크기), `/programs/` 재정리, 전월·전년 증감은 국민연금 파일이 2개월 이상 쌓여야 표시
