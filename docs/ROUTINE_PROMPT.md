...
1. `python3 scripts/report_context.py` 를 실행해 이번 주 분야(config/pledge_areas.yml, ISO 주 번호로 순환)와 코드로 센 숫자를 받는다.
   기업 사전(연결 산업 그룹의 기업 수·고용·규모·단지·입지 유형), 정부 사업(사업명·신규 여부만, 예산액 인용 금지), 대구시가 함께 추진하는 사업명,
...
6 연구기관·시장 시각(3~5편, 각 3줄, 다른 관점 포함) → 7 정책 제안(제안마다 문제·제안·근거·대상 규모·지표. 예산액·사업코드·예산서 인용 금지 — 정책제안서는 예산을 참조하지 않는다(운영자 지시 2026-09-25). 시가 할 것 / 정부에 건의할 것 / 기업·기관이 할 것 구분.
